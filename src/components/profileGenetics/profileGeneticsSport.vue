<template>
	<div class="genetics">
		<div class="row">
			<h1 class="sdv-title">Генетика / Спорт</h1>
						...
						<div class="modal-body row">
							<div class="gen-chart col-xs-12">
								<figure class="gen-chart">
									<diet-gen-chart :data="chartgendata" ></diet-gen-chart>
								</figure>
			...
			<div class="prof-genetics-data col-xs-12 col-md-9">
				<h4>Спортивные особенности</h4>
				<div class="descr-container">
					<p class="pgd-text">
						В ходе данного исследования в образце ДНК проводится анализ <b>16 полиморфизмов</b> (изменчивых участков ДНК), они незначительно меняют
						...
						<div class="snp-var-data-container col-xs-8 col-sm-8 col-md-8">
							<div v-for="snp in gene.snps" class="snp-var-data-content row">
								...
								<div class="var-data col-xs-6 col-sm-6 col-md-6">
				                    ...
				                    <div v-else>
				                      <el-tooltip class="item" effect="light" content="Для проведения анализа необходимо указать Ваш генотип" placement="bottom">
				...
				<div v-if="editTable" class="table-content col-xs-12 col-md-10">
					...
					<div class="descr-container col-xs-12">
						<p class="pgd-text">
							На основании полученных данных проведена оценка генетических
							особенностей организма в отношении физических качеств в условных баллах.
							Чем выше балл, тем выше вероятность развития признака. Баллы можно
							сравнить со средними значениями, полученными при исследовании различных
							групп населения (колонка «В среднем по популяции»), а также с баллами,
							полученными в ходе ранее проведенных генетических исследований среди
							профессиональных спортсменов (колонка «В среднем среди спортсменов»).
						</p>
					</div>
					<tblGeneticFatures></tblGeneticFatures>
					<profGenMuscle></profGenMuscle>
					<profGenMetabolism></profGenMetabolism>
					<profGenHeart></profGenHeart>
					<profGenAirProcess></profGenAirProcess>
					<profGenInsulin></profGenInsulin>
					<profGenRecovery></profGenRecovery>
				</div>
			</div>
		</div>
	</template>
...
import DietGenChart from './../DietGenChart.vue'
import profGenMuscle from './profileGeneticsMuscle.vue'
import profGenMetabolism from './profileGeneticsMetabolism.vue'
import profGenHeart from './profileGeneticsHeart.vue'
import profGenAirProcess from './profileGeneticsAirProcess.vue'
import profGenInsulin from './profileGeneticsInsulin.vue'
import profGenRecovery from './profileGeneticsRecovery.vue'
import tblGeneticFatures from './tblGeneticFatures.vue'
	...
	computed: {
		...
		filteredGenes: function(){
			return this.resultGenes.filter(function(elem) {
				return elem.snps.every(n => n.snpgroup.some(m => m.group_id === 0));
			});
		}
	},
	components: {
