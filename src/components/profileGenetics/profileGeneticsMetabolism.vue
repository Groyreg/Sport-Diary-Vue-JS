<template>
	<div class="prof-gen-feeling-full">
		<h4 @click="show = !show" class="pgf-title col-xs-12">Метаболическая эффективность мышечной 	деятельности при занятиях спортом с преимущественным проявлением выносливости
		...
		<div class="pgft-wrapper" v-show="show">
			<div class="pgf-descr-common col-xs-12">
				<p class="pgd-text">
					Во время работы скелетных мышц энергия тратится на мышечное
					сокращение, а также высвобождается в виде тепла, снижая эффективность.
					Генетические особенности организма влияют на этот процесс. «Разобщающий
					белок 2» (его кодирует ген UCP2) играет ключевую роль в этом процессе, он
					связан процессами аккумуляции жиров и рассеиванием энергии в виде тепла.
					Мутация в гене UCP2 приводит к развитию качества общей выносливости.
				</p>
				<p class="pgd-text">
					<b></b> (приведено название и возможные
  ...
  import {mapGetters} from 'vuex'
	...
	export default {
		...
		computed: {
		    ...
		    data : function () {
	            let genotype = this.userGenotypes.find(g=>g.snp_id === 14)
	            if (genotype){
	              let snp = this.snps.find(s=>s.id === genotype.snp_id);
	              let gt = this.genotypes.find(g=>g.id === genotype.genotype_id);
	              return {name:'UCP2', rs:snp.rs, variant:snp.name2, value:gt.value, description: gt.description};
	            }
